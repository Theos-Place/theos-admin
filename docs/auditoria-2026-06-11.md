...
>
> **Actualización 5 — paridad finanzas + deuda fase 1/2 (commits 1ac6757, 341e369, b33fb73):** finanzas/solicitudes con asignación igual que estudios (action 'assign', GET /api/finance/requests/assignees con rol finanzas, notificación finance_request_assigned al asignado) — DECISIÓN del usuario; las solicitudes de estudios ahora notifican a TODOS los coordinadores de estudios activos además de la lista configurable. C5 (listado de grupos sin enrollments embebidos, paginación opcional, ?include=enrollments para RecipientSelector). E4 (MemberCombobox compartido en 7 de 8 sitios; queda eventos/[id] que necesita service_history). U6 (ErrorState compartido en grupos/pagos/donaciones/becas/plantillas/vacantes), U9 (FormBuilder con Guardando…/validación), U10 (wizards dicen qué falta). C4 (tipo Member único en @/types/member; nuevo @/types/member-list; mock-members.ts y mock-member-lists.ts borrados; tipos de BD generados en src/types/database.ts — los 47 casts quedan para otra pasada). lib/format.ts (formatDate/Long/Numeric/Time + getInitials/initialsFromParts; ~37 copias migradas). **Queda (deuda menor):** los 47 casts `as unknown as` con los tipos generados, barrido masivo de contraste /40 heredado, consistencia de filtros/paginación entre listados, nonces CSP. **Áreas 1–4 esencialmente cerradas.**
>
> **Actualización 6 — casts tipados (commit 0f7e200):** `createAdminClient<Database>` tipado contra el esquema generado (`src/types/database.ts`); ahora `.select/.eq/.order`, inserts y updates se validan contra columnas reales. Eso destapó 18 laxitudes (todas corregidas: `.from(variable)` dinámico tipado como `TableName`; columnas Json y not-null en payloads casteadas a `Insertable`/`Updatable` reales; `family_unit_id` null filtrado). Los `as unknown as` de lectura bajaron de 53 a **3** (solo los selects dinámicos que el parser tipado de PostgREST no resuelve); el resto quedó en `as <DominioRow>`, que sí valida compatibilidad de forma. **Queda (deuda menor):** barrido masivo de contraste /40 heredado, consistencia de filtros/paginación entre listados, nonces CSP — todo opcional. Áreas 1–4 cerradas.
>
> **Actualización 7 — barrido de contraste (commit 86fb018):** texto informativo `/40` y `/50` → `/60` en 162 archivos (text-navy-light, text-white→/70, y los rgba equivalentes). Placeholders quedan en `/50` y estados `disabled` sin tocar (el estándar los permite en /40). Cierra el barrido masivo de contraste. **Queda (opcional, no urgente):** consistencia de filtros/paginación entre listados (requiere decisión de diseño), nonces CSP (deuda aceptada).
>
> **Actualización 8 — chips de filtro unificados (commit 3dfd16c):** nuevo `src/components/shared/FilterChips.tsx` (lenguaje visual de los chips de miembros) reemplaza los filtros ad-hoc de selección única (3 estilos distintos) en empleados, formularios, eventos (estado+tipo), finanzas (becas/donaciones/pagos) y servidores/vacantes. Cierra la inconsistencia VISUAL de filtros. **DECISIÓN del usuario:** la paginación server-side queda como proyecto aparte (cada listado de volumen — donaciones ~14k, pagos, grupos, eventos — necesita query paginada + filtros server-side + endpoint de stats porque las páginas calculan totales sobre todo el set; no es un barrido). El modal de filtros avanzados se queda solo en miembros (los demás módulos no tienen espacio de filtros que lo justifique). **Queda (opcional):** paginación server-side por listado de volumen, nonces CSP.
>
> **Actualización 9 — paginación server-side (commit 5c6267d):** donaciones (14,171 filas, el único listado de volumen real) migrado a paginación + filtros server-side + RPC `donation_stats` (migración 058) para los totales; hook `useDonations` con loadMore; verificado contra la BD. Conteos reales al 2026-06-13: el resto de listados NO justifica paginación — pagos/becas/devoluciones = 0 filas (sin integración de pagos viva); study_groups = 1,682 pero su payload ya lo arregló C5 (solo conteos, filas livianas); eventos = 12 activos / 814 totales, bajo los topes de carga (500/2000). **Pendiente real cero por ahora**; revisar si esos listados crecen. **Queda (opcional):** nonces CSP (deuda aceptada). **Auditoría 2026-06-11: cerrada.**
