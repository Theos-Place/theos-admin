...
const LEVEL = {
  ...
  bajo:  { label: 'Baja coincidencia',  cls: 'text-navy-light/70 bg-surface-low' },
}
  ...
  return (
    <div className="flex-1 min-w-0 rounded-xl border border-[var(--outline-variant)] p-3">
      ...
      <dl className="space-y-0.5 text-[12px] font-body">
        <div className="flex gap-1"><dt className="text-navy-light/70 w-16 shrink-0">Cédula</dt><dd className="text-navy-light/70 truncate">{m.cedula ?? '—'}</dd></div>
        <div className="flex gap-1"><dt className="text-navy-light/70 w-16 shrink-0">Email</dt><dd className="text-navy-light/70 truncate">{m.email ?? '—'}</dd></div>
        <div className="flex gap-1"><dt className="text-navy-light/70 w-16 shrink-0">Teléfono</dt><dd className="text-navy-light/70 truncate">{m.phone ?? '—'}</dd></div>
        <div className="flex gap-1"><dt className="text-navy-light/70 w-16 shrink-0">Nacimiento</dt><dd className="text-navy-light/70 truncate">{birthLabel(m)}</dd></div>
        <div className="flex gap-1"><dt className="text-navy-light/70 w-16 shrink-0">Creado</dt><dd className="text-navy-light/70 truncate">{formatDateNumeric(m.created_at)}</dd></div>
      </dl>
    </div>
  )
}
    ...
    <Modal onClose={onClose} titleId="fusionar-campos-title" width={768}>
      <div className="p-6 space-y-4">
        <div>
          ...
          <p className="text-[13px] text-navy-light/70 font-body mt-1">Elegí qué dato conservar de cada perfil. El perfil principal sobrevive; el otro queda inactivo.</p>
        </div>

        {/* Principal + seleccionar todo */}
        <div className="flex flex-wrap items-center gap-2 justify-between">
          <div className="flex items-center gap-2 text-xs font-body">
            <span className="text-navy-light/70">Perfil principal:</span>
            {(['a', 'b'] as const).map(s => (
          ...
          <div className="flex gap-2">
            <button onClick={() => selectAll('a')} className="text-[12px] text-coral hover:underline font-body">Todo de A</button>
            <button onClick={() => selectAll('b')} className="text-[12px] text-coral hover:underline font-body">Todo de B</button>
          </div>
        </div>

        {/* Tabla comparativa */}
        <div className="rounded-xl border border-[var(--outline-variant)] divide-y divide-[var(--outline-variant)]">
          <div className="grid grid-cols-[80px_1fr_1fr] gap-2 px-3 py-2 text-[11px] uppercase tracking-widest text-navy-light/70 font-display">
            <span>Campo</span>
          ...
          {MERGE_FIELDS.map(f => {
            ...
            return (
              <div key={f.key} className="grid grid-cols-[80px_1fr_1fr] gap-2 px-3 py-2 items-start">
                <span className="text-[12px] text-navy-light/70 font-body pt-1">{f.label}</span>
                  ...
                  return (
                    <label key={side} className={cn('flex items-start gap-2 rounded-lg px-2 py-1 cursor-pointer', choice[f.key] === side ? 'bg-coral/5' : 'hover:bg-surface-low')}>
                      ...
                      <span className="min-w-0">
                        ...
                        {ed && <span className="block text-[11px] text-navy-light/70 font-body">{ed}</span>}
                      </span>
                    </label>
                  )
                })}
        ...
        {changedFields.length > 0 && (
          <div className="rounded-xl bg-surface-low px-3 py-2.5">
            <p className="text-[11px] uppercase tracking-widest text-navy-light/70 font-display mb-1">Cambios a aplicar al principal</p>
...
export default function DuplicadosPage() {
  ...
  return (
    <div className="space-y-5">
      <Link href="/miembros" className="flex items-center gap-1 text-sm text-navy-light/70 hover:text-navy transition-colors font-body">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-navy-light/70 font-body">Pares de miembros que probablemente son la misma persona.</p>
      </div>
      ...
      {loading ? (
        <div className="py-16 text-center font-body">
          ...
          <p className="text-sm text-navy-light/70">Buscando duplicados…</p>
        </div>
      ) : visible.length === 0 ? (
            ...
            return (
              <div key={pairKey(p)} className="rounded-2xl bg-surface-card shadow-[var(--shadow-md)] p-4 space-y-3">
                <div className="flex flex-wrap items-center gap-1.5">
                  <span className={cn('rounded-full px-2.5 py-0.5 text-[12px] font-medium font-body', lvl.cls)}>
                    {lvl.label} · {p.reasons.length} campo{p.reasons.length === 1 ? '' : 's'}
                  </span>
                  {p.reasons.map(r => (
                    <span key={r} className="rounded-full bg-surface-low px-2.5 py-0.5 text-[12px] text-navy-light/70 font-body">{REASON_LABEL[r] ?? r}</span>
                  ))}
                </div>
