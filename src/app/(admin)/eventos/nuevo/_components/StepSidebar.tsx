          ...
          return (
            <Fragment key={s.num}>
              ...
              >
                <div
                  className={cn(
                    'h-6 w-6 rounded-full flex items-center justify-center text-[13px] font-bold shrink-0 transition-colors',
                    active ? 'bg-coral text-white' :
                    done   ? 'bg-teal-deep text-white' :
                    'bg-navy-light/15 text-navy-light/80',
                    'font-display',
                  )}
                >
                  {done ? '✓' : s.num}
                ...
                <span
                  className={cn(
                    ...
                    active ? 'text-navy' : done ? 'text-teal-deep' : 'text-navy-light/80',
                    'font-display',
                  )}
                >
                  {s.label}
