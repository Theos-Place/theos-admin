    ...
    <div className="page">
      ...
      {(desc || funcs || profile || skills || nivel) && (
        <div className="card py-4 px-[22px] mb-3">
          <p className="text-[11px] uppercase tracking-widest text-navy-light/80 font-display mb-2">Del puesto (no editable acá)</p>
          <div className="space-y-3">
            {nivel && <p className="text-[13px] text-navy-light/80 font-body"><span className="font-semibold text-navy">Nivel:</span> {nivel}</p>}
            {desc && <div><p className="text-[13px] font-semibold text-navy font-display">Descripción</p><p className="text-[13px] text-navy-light/80 font-body whitespace-pre-line">{desc}</p></div>}
            {funcs && <div><p className="text-[13px] font-semibold text-navy font-display">Funciones</p><p className="text-[13px] text-navy-light/80 font-body whitespace-pre-line leading-relaxed">{funcs}</p></div>}
            {profile && <div><p className="text-[13px] font-semibold text-navy font-display">Perfil</p><p className="text-[13px] text-navy-light/80 font-body whitespace-pre-line leading-relaxed">{profile}</p></div>}
            {skills && <div><p className="text-[13px] font-semibold text-navy font-display">Habilidades</p><p className="text-[13px] text-navy-light/80 font-body whitespace-pre-line leading-relaxed">{skills}</p></div>}
          </div>
        </div>
      )}
