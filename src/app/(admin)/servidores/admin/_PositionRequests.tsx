            ...
            <div key={r.id} className="rounded-xl border border-[var(--outline-variant)]">
              <div className="flex items-center gap-2 px-4 py-2.5">
                ...
                <button onClick={() => resolve(r.id, 'reject')} disabled={!!busy}
                  className="inline-flex items-center gap-1 rounded-full border border-[var(--outline-variant)] px-3 py-1.5 text-[13px] text-navy-light hover:text-coral hover:bg-coral/5 transition-colors disabled:opacity-50 font-body">
                  <X size={13} /> Rechazar
                </button>
                <button onClick={() => resolve(r.id, 'approve')} disabled={!!busy}
                  className="inline-flex items-center gap-1 rounded-full bg-coral px-3 py-1.5 text-[13px] text-white hover:bg-coral-deep transition-colors disabled:opacity-50 font-body">
              ...
              {isOpen && hasDetail && (
                <div className="px-4 pb-3 pt-1 space-y-2 border-t border-[var(--outline-variant)]">
                  {r.description && <div><p className="text-[11px] uppercase tracking-widest text-navy-light/80 font-display">Descripción</p><p className="text-[13px] text-navy-light/80 font-body">{r.description}</p></div>}
                  {r.functions && <div><p className="text-[11px] uppercase tracking-widest text-navy-light/80 font-display">Funciones</p><p className="text-[13px] text-navy-light/80 font-body whitespace-pre-line">{r.functions}</p></div>}
                  {r.profile && <div><p className="text-[11px] uppercase tracking-widest text-navy-light/80 font-display">Perfil</p><p className="text-[13px] text-navy-light/80 font-body whitespace-pre-line">{r.profile}</p></div>}
                </div>
              )}
            </div>
          )
