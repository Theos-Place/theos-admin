  ...
  return (
    <div id="solicitudes" className="rounded-2xl bg-surface-card p-5 shadow-[var(--shadow-md)]">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-medium text-amber-700 font-body">{reqs.length} pendiente{reqs.length !== 1 ? 's' : ''}</span>
      </div>
            ...
            <div key={r.id} className="rounded-xl border border-[var(--outline-variant)]">
              <div className="flex items-center gap-2 px-4 py-2.5">
                <button onClick={() => hasDetail && setOpen(isOpen ? null : r.id)} className="flex-1 min-w-0 text-left">
                  ...
                  <p className="text-[11px] text-navy-light/60 font-body truncate">
              ...
              {isOpen && hasDetail && (
                <div className="px-4 pb-3 pt-1 space-y-2 border-t border-[var(--outline-variant)]">
                  {r.description && <div><p className="text-[10px] uppercase tracking-widest text-navy-light/60 font-display">Descripción</p><p className="text-[13px] text-navy-light/80 font-body">{r.description}</p></div>}
                  {r.functions && <div><p className="text-[10px] uppercase tracking-widest text-navy-light/60 font-display">Funciones</p><p className="text-[13px] text-navy-light/80 font-body whitespace-pre-line">{r.functions}</p></div>}
                  {r.profile && <div><p className="text-[10px] uppercase tracking-widest text-navy-light/60 font-display">Perfil</p><p className="text-[13px] text-navy-light/80 font-body whitespace-pre-line">{r.profile}</p></div>}
                </div>
              )}
            </div>
          )
