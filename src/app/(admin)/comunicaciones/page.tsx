  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-5" style={{ background: 'var(--surface-card)', boxShadow: 'var(--shadow-md)' }}>
        <p className="text-[10px] uppercase tracking-widests text-navy-light/40 mb-4" style={{ fontFamily: 'var(--font-display)' }}>
          Actividad — últimos 7 días
        </p>
        <div className="flex items-end gap-2 h-20">
          {activityData.map((d, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
              <div className="w-full relative flex items-end justify-center" style={{ height: 56 }}>
                <div
                ...
                />
                {d.count > 0 && (
                  <span className="absolute -top-5 text-[10px] font-bold text-coral" style={{ fontFamily: 'var(--font-mono)' }}>
                    {d.count}
                  </span>
                )}
              </div>
              <span className="text-[10px] text-navy-light/40 capitalize" style={{ fontFamily: 'var(--font-body)' }}>{d.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Tabs */}
      <div>
        <div className="flex gap-1 border-b mb-4" style={{ borderColor: 'var(--outline-variant)' }}>
        ...
        {tab === 'historial' && (
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {filtered.length === 0 ? (
                <div className="rounded-2xl py-12 flex flex-col items-center gap-3" style={{ background: 'var(--surface-card)' }}>
                  <CheckCircle2 size={24} className="text-navy-light/20" />
                  <p className="text-sm text-navy-light/40" style={{ fontFamily: 'var(--font-body)' }}>No hay mensajes con ese filtro.</p>
                </div>
              ) : (
                filtered.map(msg => (
                  <div
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-2">
                          ...
                          <span
                            className={cn('rounded-full px-2 py-0.5 text-[10px] font-semibold', STATUS_STYLE[msg.status])}
                      ...
                      <Link
                        ...
                        className="shrink-0 flex items-center gap-1 rounded-lg border px-3 py-1.5 text-[12px] text-navy-light hover:bg-surface-low transition-colors"
                    ...
                    <div className="mt-3 pt-3 border-t flex items-center justify-between flex-wrap gap-2" style={{ borderColor: 'var(--outline-variant)' }}>
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-navy-light/40" style={{ fontFamily: 'var(--font-body)' }}>
          ...
          <div className="space-y-3">
            {drafts.length === 0 ? (
              <div className="rounded-2xl py-12 flex flex-col items-center gap-3" style={{ background: 'var(--surface-card)' }}>
                ...
                <p className="text-sm text-navy-light/40" style={{ fontFamily: 'var(--font-body)' }}>No hay borradores guardados.</p>
              </div>
            ) : (
              drafts.map(msg => (
                <div
                ...
                >
                  ...
                  <Link
                    href={`/comunicaciones/nueva`}
                    className="shrink-0 inline-flex items-center gap-1.5 rounded-full bg-coral px-3.5 py-1.5 text-[12px] text-white hover:bg-coral-deep transition-colors"
                    style={{ fontFamily: 'var(--font-body)' }}
                  >
                    <FileEdit size={12} />
                    Continuar editando
