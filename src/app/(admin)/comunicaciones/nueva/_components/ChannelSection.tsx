      ...
      <div className="flex gap-2">
        {([
          ...
          <button
            ...
            className={cn(
              'flex-1 flex flex-col items-center gap-1.5 rounded-xl border p-3 text-[13px] font-medium transition-all font-body',
              opt.disabled
                ? 'opacity-50 cursor-not-allowed text-navy-light/80'
                : channel === opt.key ? 'bg-navy border-navy text-white' : 'text-navy-light/80 hover:text-navy'
            )}
          ...
          >
            ...
            {opt.label}
            {opt.disabled && (
              <span className="text-[11px] font-normal text-navy-light/80 font-body">Próximamente</span>
            )}
          </button>
        ))}
      </div>
