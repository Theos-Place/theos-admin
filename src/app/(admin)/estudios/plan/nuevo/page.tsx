...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <label className="flex items-center justify-between gap-4 cursor-pointer">
      <span className="text-sm text-navy-light/80 font-body">{label}</span>
      <button
        type="button"
        role="switch"
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-5 space-y-4 bg-surface-card shadow-[var(--shadow-md)]">
        ...
        {[
          ...
          <div key={group.label}>
            <p className="text-[13px] text-navy-light/80 mb-2 font-display">{group.label}</p>
      ...
      <div className="flex items-center gap-3 pb-6">
        ...
        <Link
          href="/estudios/plan"
          className="text-sm text-navy-light/80 hover:text-navy-light transition-colors ml-2 font-body"
        >
          Cancelar
        </Link>
      </div>
