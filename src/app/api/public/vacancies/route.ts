...
// requireRoles. Expone SOLO puestos publicados ('published'/'aprobado') y con
// una WHITELIST de campos de cartelera — nunca datos de aplicantes ni internos.
// Rate limit por IP.
export async function GET(req: Request) {
  try {
    ...
    // 'aprobado' (flujo de solicitud) y 'published' (legacy) son ambos
    // "visible y aplicable" — mismo criterio que el GET admin ?published=1.
    const publicadas = all.filter(v => v.status === 'published' || v.status === 'aprobado')
