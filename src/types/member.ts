...
export type Member = {
  ...
  study_history?: Array<{ group_id: string | null; enrollment_id: string; code: string; name: string; date: string | null; year: number | null; weeks: number | null; status: string; /** Estado del GRUPO, para no pedir pago de un estudio cuyo grupo ya cerró. */ group_status?: string | null; requires_payment: boolean; payment_status: string | null; payments_count?: number; cost: number; grade: number | null; notes: string | null; es_externo: boolean; fuente_externa: string | null; registrado_por: string | null }>
