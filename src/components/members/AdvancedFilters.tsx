...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <p className="mb-1.5 text-[11px] tracking-widest uppercase text-navy-light/80 font-display">
      {children}
    </p>
  )
}
    ...
    <div className="flex gap-3 flex-wrap">
      {options.map(opt => (
        <label key={opt.value} className="flex items-center gap-1.5 cursor-pointer">
          ...
          />
          <span className="text-xs text-navy-light/80 select-none font-body">
            {opt.label}
          </span>
        </label>
      ))}
...
function ConditionsList({
  ...
  return (
    ...
    >
      <p className="mb-2 text-[11px] tracking-widest uppercase text-navy-light/80 font-display">
        Filtros activos
      </p>
      {filtered.length === 0 ? (
        <p className="text-xs text-navy-light/80 italic font-body">
          Ninguno
        </p>
      ) : (
          ...
          {filtered.map(c => (
            <div key={c.id} className="flex items-center justify-between gap-2 rounded-lg bg-surface-card px-2.5 py-1.5">
              ...
              <button
                ...
                className="shrink-0 text-navy-light/80 hover:text-coral transition-colors"
                aria-label="Quitar"
              >
        ...
        <div className="flex rounded-xl bg-surface-low p-1 gap-1">
          <button
            ...
            className={cn(
              ...
              mode === 'taken' ? 'bg-navy text-white' : 'text-navy-light/80 hover:text-navy',
            )}
          >
            Ha llevado
          </button>
          <button
            ...
            className={cn(
              'flex-1 rounded-lg px-3 py-1.5 text-xs font-medium font-display transition-all',
              mode === 'not_taken' ? 'bg-navy text-white' : 'text-navy-light/80 hover:text-navy',
            )}
          >
            No ha llevado
          </button>
      ...
      {eventType === 'charla' && (
        <div>
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-[11px] uppercase tracking-widest text-navy-light/80 pt-1 font-display">
              Sedes históricas
            </p>
      ...
      {permite('age') && <div>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-full rounded-xl bg-surface-low px-3 py-2 text-sm text-navy placeholder-navy-light/50 outline-none focus:ring-1 focus:ring-coral/30 font-body"
          />
          <span className="text-navy-light/80 shrink-0 text-sm">–</span>
          <input
          ...
          <button
            ...
            className={cn(
              ...
              activeTab === tab.key
                ...
                : 'text-navy-light/80 hover:text-navy-light',
              'font-body',
            )}
          >
            {tab.label}
          ...
          {activeTab === 'attend'  && (
            <div className="space-y-6">
              ...
              <div className="border-t border-[var(--outline-variant)] pt-5">
                <p className="mb-3 text-[11px] tracking-widest uppercase text-navy-light/80 font-display">
