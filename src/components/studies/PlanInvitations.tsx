...
const STATUS_LABEL: Record<Invitation['status'], { label: string; cls: string }> = {
  ...
  revoked: { label: 'Revocada', cls: 'bg-surface-low text-navy-light/60' },
}
...
export function PlanInvitations({ planId }: { planId: string }) {
  ...
  return (
    <div className="rounded-2xl bg-surface-card p-5 shadow-[var(--shadow-md)] space-y-4">
      ...
      {adding && <p className="text-[12px] text-navy-light/60 font-body">Invitando…</p>}

      <div className="space-y-1.5">
        {loading ? (
          <p className="text-[12px] text-navy-light/60 font-body">Cargando…</p>
        ) : invitations.length === 0 ? (
          <p className="text-[12px] text-navy-light/60 font-body">Todavía no hay invitados.</p>
        ) : invitations.map(inv => (
          <div key={inv.id} className="flex items-center gap-3 rounded-xl px-3 py-2 hover:bg-surface-low transition-colors">
            <div className="h-8 w-8 rounded-full bg-navy flex items-center justify-center text-[10px] font-bold text-white shrink-0">
              {getInitials(inv.member_name)}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm text-navy truncate font-body">{inv.member_name}</p>
              {inv.invited_by_name && <p className="text-[11px] text-navy-light/60 font-body">Invitado por {inv.invited_by_name}</p>}
            </div>
            <span className={cn('rounded-full px-2 py-0.5 text-[10px] font-semibold font-display', STATUS_LABEL[inv.status].cls)}>
