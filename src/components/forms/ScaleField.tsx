    ...
    <div className="w-full space-y-1.5">
          ...
          if (readOnly) {
            return (
              <div
                key={n}
                aria-hidden
                className={cn(base, 'flex items-center justify-center border-[var(--outline-variant)] text-navy-light/60')}
              >
                {n}
              </div>
            )
      ...
      {(minLabel || maxLabel) && (
        <div className="flex w-full items-start justify-between gap-3">
          <span className="max-w-[45%] text-[11px] leading-tight text-navy-light/60 font-body">
            {minLabel}
          </span>
          <span className="max-w-[45%] text-[11px] leading-tight text-navy-light/60 font-body text-right">
            {maxLabel}
          </span>
        </div>
      )}
