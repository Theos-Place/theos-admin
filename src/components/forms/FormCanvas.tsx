    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-80 rounded-2xl border-2 border-dashed gap-3 border-[var(--outline-variant)]">
        <div className="h-12 w-12 rounded-xl bg-navy/5 flex items-center justify-center">
          <GripVertical size={22} className="text-navy-light/70" />
        </div>
        <p className="text-sm text-navy-light/70 text-center max-w-xs font-body">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <GripVertical size={15} className="text-navy-light/70 cursor-grab shrink-0" />
                <FileText size={14} className="text-blue-400 shrink-0" />
                <div className="flex-1">
                  <span className="text-[12px] font-bold text-blue-500 uppercase tracking-widest font-display">
                    INICIO DE PÁGINA {pgNum}
                  </span>
                  {field.label && (
                    <span className="ml-2 text-[12px] text-navy-light/70 font-body">· {field.label}</span>
                  )}
                </div>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button type="button" onClick={e => { e.stopPropagation(); onSelectField(field.id) }} className="relative after:absolute after:content-[''] after:-inset-1.5 h-7 w-7 rounded-lg hover:bg-navy/10 flex items-center justify-center transition-colors" aria-label="Editar campo">
                    <Pencil size={12} className="text-navy-light/70" />
                  </button>
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="h-6 rounded-lg px-2 text-[12px] hover:bg-teal-deep/10 transition-colors text-[var(--brand-teal-deep,#2a8b8f)] font-body"
                  >
                    Editar
                  </button>
                  <button
              ...
              <div className="flex flex-wrap gap-1">
                {selectedLabels.length > 0
                  ? selectedLabels.map(label => (
                    <span
                      key={label}
                      className="text-[11px] py-0.5 px-2 rounded-full bg-[rgba(112,189,194,.18)] text-[var(--brand-teal-deep,#2a8b8f)] font-medium font-body"
                    >
                      {label}
                    </span>
                  ))
                  : <span className="text-[12px] text-[var(--fg-muted,#8c8fb0)] font-body">Sin campos seleccionados — hacé clic en Editar</span>
                }
              </div>
          ...
          >
            ...
            <div className={cn('absolute top-2 right-2 flex items-center gap-1 transition-opacity z-10', 'opacity-0 group-hover:opacity-100')}>
              {logicCount > 0 && (
                ...
                >
                  ...
                  <span className="text-[11px] font-bold text-amber-600">{logicCount}</span>
                </button>
              )}
              ...
              <button type="button" onClick={e => { e.stopPropagation(); onDuplicateField(field.id) }} className="relative after:absolute after:content-[''] after:-inset-1.5 h-7 w-7 rounded-lg hover:bg-navy/10 flex items-center justify-center transition-colors" title="Duplicar" aria-label="Duplicar campo">
                <Copy size={12} className="text-navy-light/70" />
              </button>
            ...
            <div className="flex items-start gap-3">
              <div className="mt-0.5 cursor-grab active:cursor-grabbing shrink-0">
                <GripVertical size={16} className="text-navy-light/70 hover:text-navy-light/70 transition-colors" />
              </div>
                ...
                <div className="flex-1 space-y-2 min-w-0">
                  <div className="flex items-start gap-2">
                    <div className="shrink-0 h-5 w-5 rounded flex items-center justify-center mt-0.5 bg-surface-low">
                      <FieldTypeIcon type={field.type} size={11} className="text-navy-light/70" />
                    </div>
                    <div className="flex-1 min-w-0 flex items-start gap-2">
                      <p className="text-[13px] font-semibold text-navy leading-snug flex-1 font-display">
                        {field.label || (
                          ...
                          <span className="text-navy-light/70 italic">
                            {field.type === 'info' ? 'Texto informativo' : 'Sin etiqueta'}
                          </span>
                        )}
                        {field.is_required && <span className="ml-1 text-coral text-[12px]">*</span>}
                      </p>
                      {logicCount > 0 && (
                        <button
                          type="button"
                  ...
                  {field.helper_text && (
                    <p className="text-[12px] text-navy-light/70 ml-7 font-body">{field.helper_text}</p>
                  )}
              ...
              <span className="shrink-0 mt-0.5 h-5 min-w-5 rounded-full bg-navy/5 flex items-center justify-center text-[11px] text-navy-light/70 font-mono px-1">
                {index + 1}
              </span>
            </div>
