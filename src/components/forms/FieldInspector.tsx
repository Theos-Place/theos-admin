...
export function FieldInspector({ field, allFields, onChange, onFocusLogic }: FieldInspectorProps) {
    ...
    return (
      <div className="p-4 space-y-4">
        ...
        <div className="flex gap-2">
          <button
            type="button"
            className="flex-1 rounded-xl border py-1.5 text-[13px] text-navy-light hover:bg-surface-low transition-colors border-[var(--outline-variant)] font-body"
        ...
        {GROUPS.map(group => (
          <div key={group}>
            <p className="text-[11px] font-bold uppercase tracking-widest text-navy-light/80 mb-2 font-display">
              {group}
            </p>
            <div className="space-y-0.5">
              {PERSONAL_DATA_FIELDS.filter(f => f.group === group).map(pf => (
                <label key={pf.key} className="flex items-center gap-2.5 py-1.5 cursor-pointer">
                  ...
                  />
                  <span className="text-[13px] text-navy font-body">{pf.label}</span>
                </label>
              ))}
            </div>
          </div>
  ...
  const sectionBtn = (key: typeof activeSection, label: string) => (
    <button
      ...
      className={cn(
        'flex-1 py-2 text-[13px] font-medium transition-all border-b-2 -mb-px font-display',
        activeSection === key ? 'border-coral text-navy' : 'border-transparent text-navy-light/80 hover:text-navy'
      )}
    >
      {label}
    </button>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex border-b px-2 shrink-0 border-[var(--outline-variant)]">
        ...
        <button
          ...
          className={cn(
            'flex-1 py-2 text-[13px] font-medium transition-all border-b-2 -mb-px flex items-center justify-center gap-1 font-display',
            activeSection === 'logic' ? 'border-coral text-navy' : 'border-transparent text-navy-light/80 hover:text-navy'
          )}
        >
          Lógica
          {logicCount > 0 && (
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="flex items-center justify-between pt-1">
              <div>
                ...
                <p className="text-[13px] text-navy-light/80 font-body">Marcado con asterisco</p>
              </div>
              <div
              ...
              <div className="flex gap-2">
                {([[1, 5], [1, 10]] as const).map(([min, max]) => (
                  <button
                    ...
                    className={cn('flex-1 rounded-xl border py-2 text-[13px] font-medium transition-colors font-display', field.scale_min === min && field.scale_max === max ? 'bg-coral text-white border-coral' : 'text-navy-light/80 hover:bg-surface-low')}
          ...
          <div className="p-4 space-y-4">
            ...
            {(field.logic_rules ?? []).length === 0 && (
              <div className="rounded-xl border-2 border-dashed py-6 flex flex-col items-center gap-2 border-[var(--outline-variant)]">
                <p className="text-[13px] text-navy-light/80 font-body">Sin reglas aún</p>
              </div>
            )}
            ...
            {(field.logic_rules ?? []).map((rule, ruleIdx) => (
              <div key={rule.id} className="rounded-xl border space-y-3 p-3 border-[var(--outline-variant)]">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-[13px] font-semibold text-navy-light/80 font-display">
                    Regla {ruleIdx + 1}
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[13px] text-navy-light/80 shrink-0 font-body">Acción</span>
                  <select
                    className={cn(inputCls, 'flex-1')}
                    value={rule.action}
                ...
                {/* Combinator */}
                <div className="flex items-center gap-2">
                  <span className="text-[13px] text-navy-light/80 shrink-0 font-body">Combinar</span>
                  <div className="flex gap-1">
                    {(['AND', 'OR'] as const).map(op => (
                      <button
                        ...
                        className={cn('rounded-lg px-2.5 py-1 text-[13px] font-semibold transition-all font-display', rule.condition_operator === op ? 'bg-navy text-white' : 'text-navy-light/80 hover:text-navy')}
                      >
                        {op}
                      </button>
                    ))}
                  ...
                  {rule.conditions.map((cond, ci) => {
                    ...
                    return (
                      <div key={cond.id} className="rounded-lg p-2.5 space-y-2 bg-surface-low">
                        {ci > 0 && (
                          <p className="text-[11px] font-bold text-navy-light/80 text-center font-display">
                        ...
                        <div className="flex items-center justify-between">
                          <button type="button" onClick={() => addCondition(rule.id)} className="text-[13px] text-coral hover:text-coral-deep transition-colors flex items-center gap-1 font-body">
            ...
            <button
              type="button"
              onClick={addRule}
              className="w-full flex items-center justify-center gap-1.5 rounded-xl border-2 border-dashed py-2.5 text-[13px] text-coral hover:bg-coral/5 transition-colors border-[rgb(255_107_74_/_0.3)] font-body"
            >
              <Plus size={13} />
              Agregar regla
            </button>
            ...
            {priorFields.length === 0 && (
              <p className="text-[13px] text-navy-light/80 text-center font-body">
