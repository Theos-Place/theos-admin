...
export function DeliveryStats({ message }: Props) {
  ...
  const cards = [
    ...
    },
    {
      ...
      value: stats.failed,
      icon: XCircle,
      color: stats.failed > 0 ? 'text-coral' : 'text-navy-light/40',
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map(({ label, value, icon: Icon, color, bg }) => (
          <div key={label} className="rounded-2xl p-4" style={{ background: 'var(--surface-card)', boxShadow: 'var(--shadow-md)' }}>
            ...
            <p className="text-[10px] uppercase tracking-widests text-navy-light/40 mb-1" style={{ fontFamily: 'var(--font-display)' }}>
              {label}
            </p>
            <p className={cn('text-2xl font-extrabold tabular-nums', color)} style={{ fontFamily: 'var(--font-display)' }}>
              {value}
            </p>
          </div>
        ))}
        ...
        <div className="rounded-2xl p-5 space-y-3" style={{ background: 'var(--surface-card)', boxShadow: 'var(--shadow-md)' }}>
          ...
          <div className="space-y-2.5">
            <div className="flex items-center gap-3">
              <span className="text-[11px] text-navy-light/60 w-20 shrink-0" style={{ fontFamily: 'var(--font-body)' }}>Entregados</span>
              ...
              <span className="text-[11px] font-mono text-navy tabular-nums w-10 text-right" style={{ fontFamily: 'var(--font-mono)' }}>
                {deliveryRate}%
              </span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-[11px] text-navy-light/60 w-20 shrink-0" style={{ fontFamily: 'var(--font-body)' }}>Fallidos</span>
              <div className="flex-1 h-2 rounded-full bg-navy/10">
            ...
            {message.channel === 'both' && (
              <>
                <div className="flex items-center gap-3">
                  <span className="text-[11px] text-navy-light/60 w-20 shrink-0" style={{ fontFamily: 'var(--font-body)' }}>WhatsApp</span>
                  <div className="flex-1 h-2 rounded-full bg-navy/10">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-[11px] text-navy-light/60 w-20 shrink-0" style={{ fontFamily: 'var(--font-body)' }}>Email</span>
                  <div className="flex-1 h-2 rounded-full bg-navy/10">
