  ...
  return (
    <div className={cn('rounded-2xl bg-surface-card p-4 sm:p-5 shadow-[var(--shadow-md)]', className)}>
      <div className="mb-3">
        ...
        {subtitle && <p className="text-[12px] text-navy-light/70 font-body mt-0.5">{subtitle}</p>}
      </div>
      {empty ? (
      ...
      {!empty && footnote && (
        <p className="mt-2.5 text-[11px] text-navy-light/70 font-body leading-snug">{footnote}</p>
      )}
    </div>
  )
}
