/** SOLO LECTURA: pagos y matrículas de Gisselle López Rodríguez. */
...
;(async () => {
  ...
  const f = await c.query(`
    ...
    from members
    where unaccent(lower(first_name||' '||last_name)) like '%gisselle%lopez%'
       or unaccent(lower(first_name||' '||last_name)) like '%gissell%lopez%'
       or unaccent(lower(first_name||' '||last_name)) like '%gisele%lopez%'
  ...
  const p = await c.query(`
    select pa.id, pa.member_id, pa.amount, pa.currency, pa.status, pa.concept, pa.description,
           ...
           pa.study_group_id, g.name as grupo, pa.receipt_path, pa.paid_at,
           pa.created_at, pa.updated_at, pa.reviewed_by, pa.reviewed_at, pa.payment_plan_id,
           pa.installment_number, pa.reference_code, pa.sinpe_confirmation
  ...
  const e = await c.query(`
    select e.id, e.member_id, e.status, e.enrolled_at, e.dropped_at, e.created_at,
           g.name as grupo, pl.name as plan, pl.cost
    from study_enrollments e join study_groups g on g.id=e.group_id
      left join study_plans pl on pl.id = g.plan_id
    where e.member_id = any($1) and e.created_at > now() - interval '1 year'
    order by e.created_at`, [ids])
  console.log(`\n=== MATRÍCULAS del último año (${e.rowCount}) ===`)
