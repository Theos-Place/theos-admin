-- Fix de merge_members (backlog: "family_unlink_requests zombie" + colisión de
-- event_checkins). Dos correcciones sobre la versión de la mig. 092:
--   1. La mig. 135 dropeó family_unlink_requests, pero merge_members todavía la
--      referenciaba en dos UPDATE → el RPC fallaba con "relation does not exist"
--      y revertía TODA la fusión. Se quitan esas dos líneas.
--   2. event_checkins tiene UNIQUE(member_id, event_id) parcial (mig. 067) pero
--      merge_members reasignaba sus filas sin pre-borrar las que chocaban cuando
--      keep y dup asistieron al MISMO evento → "duplicate key". Se agrega el
--      pre-borrado de colisiones, igual que ya se hace con event_registrations.
